...
TEST(Page, Struct)
{
    EXPECT_EQ(sizeof(struct _PageMeta), 20);
    EXPECT_EQ(sizeof(struct _PageStruct), PAGE_LEN);
}
...
{
    EXPECT_EQ(StorageMacroblock::PAGES_COUNT, ((STORAGE_PAGE_PAYLOAD_SIZE * 8) / (sizeof(struct Header::_MetaUnit) * 8 + 2)) + StorageMacroblock::RESERVED_PAGES_COUNT);
}
...
TEST_F(StorageFixture, BadSaveRequest)
{
    ...
    EXPECT_EQ(sat->save(address, emptyStr, 0, nullptr, STORAGE_PAGE_SIZE * 4), STORAGE_ERROR);
    EXPECT_EQ(sat->save(address, emptyStr, 0, nullptr, STORAGE_PAGE_SIZE * 4000), STORAGE_ERROR);
    EXPECT_EQ(sat->save(address, nullptr, 0, nullptr, 0), STORAGE_ERROR);
    EXPECT_EQ(sat->save(address, nullptr, 0, nullptr, STORAGE_PAGE_SIZE * 4), STORAGE_ERROR);
    EXPECT_EQ(sat->save(address, nullptr, 0, nullptr, STORAGE_PAGE_SIZE * 4000), STORAGE_ERROR);
}

TEST_F(StorageFixture, BadLoadRequest)
{
    EXPECT_EQ(sat->load(address, nullptr, 0), STORAGE_ERROR);
    EXPECT_EQ(sat->load(address, nullptr, STORAGE_PAGE_SIZE * 4), STORAGE_ERROR);
    EXPECT_EQ(sat->load(address, nullptr, STORAGE_PAGE_SIZE * 4000), STORAGE_ERROR);
}
...
TEST_F(StorageFixture, CheckDataMiddlePageLoad)
{
    ...
    EXPECT_EQ(page.load(/*startPage=*/true), STORAGE_ERROR);
    EXPECT_EQ(page.load(), STORAGE_OK);
    EXPECT_FALSE(memcmp(page.page.payload, wdata + STORAGE_PAGE_PAYLOAD_SIZE, sizeof(page.page.payload)));
    EXPECT_FALSE(page.isStart());
    EXPECT_TRUE(page.isMiddle());
    EXPECT_FALSE(page.isEnd());
    EXPECT_TRUE(page.validatePrevAddress());
...
TEST_F(StorageFixture, CheckDataEndPageLoad)
{
    ...
    EXPECT_EQ(page.load(/*startPage=*/true), STORAGE_ERROR);
    EXPECT_EQ(page.load(), STORAGE_OK);
    EXPECT_FALSE(memcmp(page.page.payload, wdata + STORAGE_PAGE_PAYLOAD_SIZE * 2, sizeof(page.page.payload)));
    EXPECT_FALSE(page.isStart());
    EXPECT_FALSE(page.isMiddle());
    EXPECT_TRUE(page.isEnd());
    EXPECT_TRUE(page.validatePrevAddress());
...
TEST_F(StorageFixture, AutomaticHeadersCreation)
{
    ...
    EXPECT_EQ(sat->find(FIND_MODE_EQUAL, &address, shortPrefix, 1), STORAGE_OK);
    EXPECT_EQ(address, StorageMacroblock::RESERVED_PAGES_COUNT * STORAGE_PAGE_SIZE);
}
...
TEST_F(StorageFixture, FindAllData)
{
    ...
    EXPECT_EQ(address, STORAGE_PAGE_SIZE * StorageMacroblock::RESERVED_PAGES_COUNT);
    EXPECT_EQ(sat->find(FIND_MODE_MAX, &address, shortPrefix), STORAGE_OK);
    EXPECT_EQ(address, STORAGE_PAGE_SIZE * (StorageAT::getStoragePagesCount() - 1));
