...
StorageAT - библиотека таблицы распределения данных (файловая система) в памяти для embeded устройств. 

Навигация происходит посредством испольлзования префикса и идентификатора, принадлежащих каждой записи. Префикс используется для яопределения назначения данных, а идентификатор - для их нумерации.

## Структура таблицы

Память делится на макроблоки равного размера (см. Рисунок 1).
Навигация по макроблокам осуществлется с помощью индексов, начиная с 0.
...
Каждый макроблок имеет оглавление (Header) и область хранения данных (User data), их размеры фиксированны. Оглавление и область хранения данных делятся на фиксированное количество страниц по 256 байт каждая. Навигация по области хранения данных осуществляется с помощью индексов, начиная с 0. Разметка приведена на Рисунке 2.
...
Для любых манипуляцмй с памятью используется 32-битный адрес, поэтому для сохранения и загрузки данных сначала необходимо выполнить поиск в соответствующем режиме.
...
### 3. Примеры использования таблицы
